...
async def test_add_items_success(
    ...
    results = await redis_editor.search(query="Memory A", top_k=5, user_id=sample_memory_item.user_id)
...
async def test_search_with_similarity_threshold_filters_results(
    ...
    exact = await redis_editor.search(query="query_a", top_k=2, user_id=user_id, similarity_threshold=0.01)
    assert len(exact) == 1
    assert exact[0].memory == "Memory A"

    # Loose: both items survive
    all_results = await redis_editor.search(query="query_a", top_k=2, user_id=user_id, similarity_threshold=10.0)
